  ...
  return (
    <div style={{ maxWidth: '1080px', margin: '0 auto', width: '100%' }}>
      <div style={{ marginBottom: '1rem' }}>
        <h1 style={{ fontSize: '20px', fontWeight: 300, color: 'var(--stripe-navy)', margin: '0 0 0.5rem 0', letterSpacing: '-0.64px', fontFeatureSettings: '"ss01"' }}>Automated Sequences</h1>
        <p style={{ color: 'var(--stripe-body)', fontSize: '12px', margin: 0, fontWeight: 300, fontFeatureSettings: '"ss01"' }}>Build and manage multi-step follow-up sequences.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '1rem' }}>
        {[
          ...
          <div key={i} style={{ backgroundColor: '#ffffff', border: '1px solid var(--stripe-border)', borderRadius: '6px', padding: '1.25rem', boxShadow: 'var(--stripe-shadow-ambient)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
              <div style={{ fontSize: '12px', color: 'var(--stripe-navy)', fontWeight: 500 }}>{drip.name}</div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--stripe-body)', marginBottom: '1rem' }}>{drip.trigger}</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid var(--stripe-border)', paddingTop: '1rem', fontFeatureSettings: '"tnum"' }}>
              <div>
                ...
                <div style={{ fontSize: '12px', color: 'var(--stripe-navy)', fontWeight: 500 }}>{drip.steps}</div>
              </div>
              <div>
                <div style={{ fontSize: '12px', color: 'var(--stripe-label)' }}>Conversion</div>
                <div style={{ fontSize: '12px', color: 'var(--stripe-navy)', fontWeight: 500 }}>{drip.conversion}</div>
              </div>
            </div>
          </div>
        ))}
        ...
        <div style={{ backgroundColor: '#f6f9fc', border: '1px dashed var(--stripe-purple)', borderRadius: '6px', padding: '1.25rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', minHeight: '180px' }}>
          ...
          <div style={{ fontSize: '12px', color: 'var(--stripe-purple)', fontWeight: 500 }}>Create Sequence</div>
        </div>
      </div>
